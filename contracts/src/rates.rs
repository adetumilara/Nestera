...
pub fn set_lock_rate(env: &Env, duration_days: u64, rate: i128) -> Result<(), SavingsError> {
    ...
    env.storage().instance().set(&DataKey::LockRate(duration_days), &rate);
    Ok(())
}
...
pub fn get_flexi_rate(env: &Env) -> i128 {
    env.storage().instance().get(&DataKey::FlexiRate).unwrap_or(0)
}

pub fn get_goal_rate(env: &Env) -> i128 {
    env.storage().instance().get(&DataKey::GoalRate).unwrap_or(0)
}

pub fn get_group_rate(env: &Env) -> i128 {
    env.storage().instance().get(&DataKey::GroupRate).unwrap_or(0)
}

pub fn get_lock_rate(env: &Env, duration_days: u64) -> Result<i128, SavingsError> {
    env.storage().instance().get(&DataKey::LockRate(duration_days)).ok_or(SavingsError::PlanNotFound)
}
...
pub fn calculate_flexi_interest(balance: i128, rate: i128, duration_seconds: u64) -> i128 {
    ...
    let numerator = balance
        .checked_mul(rate).unwrap_or(0)
        .checked_mul(duration_seconds as i128).unwrap_or(0);
